  ...
  try {
    ...
    app.post("/make-admin", async (req, res) => {
      ...
      try {
        ...
        res
          .status(200)
          .send({ message: "User has been promoted to admin successfully" });
      } catch (error) {
    ...
    app.get("/upcoming-meals", async (req, res) => {
      ...
      try {
        ...
        const upcomingMeals = await upcomingMealCollection
          .find({})
          .sort({ likes: -1 })
          .toArray();
      ...
      try {
        ...
        if (user.badge === "Bronze") {
          return res.status(403).send({
            message:
              "You need a subscription to like meals. Please upgrade your badge.",
          });
        }
        ...
        if (meal.likes >= 10) {
          ...
          if (result.insertedId) {
            ...
            await upcomingMealCollection.deleteOne({
              _id: new ObjectId(mealId),
            });

            return res.status(201).send({
              message: "Meal added successfully from upcoming meal",
    ...
    app.get("/meal-requests", async (req, res) => {
      try {
        ...
        const totalMealRequests = await mealRequestCollection.countDocuments(
          filter
        );
    ...
    app.get("/meal-requests-user", async (req, res) => {
      try {
        ...
        res.status(200).send({
          message: "Meal requests retrieved successfully",
          mealRequests: updatedMealRequests,
          totalRequest,
    ...
    app.post("/review-delete", async (req, res) => {
      try {
        ...
        const reviewDeleteResult = await reviewCollection.deleteOne({
        ...
        if (mealUpdateResult.modifiedCount === 0) {
          return res.status(404).json({
            message:
              "Meal not found or review not present in meal or already deleted",
          });
        }
    ...
    app.post("/review-update", async (req, res) => {
      try {
        ...
        if (
          reviewUpdateResult.modifiedCount > 0 &&
          mealUpdateResult.modifiedCount > 0
        ) {
          return res
            .status(200)
    ...
    app.delete("/meal-requests-user", async (req, res) => {
      try {
        ...
        if (!id) {
          return res
            .status(400)
            .send({ message: "Meal request ID is required" });
        }
    ...
    app.patch("/meal-requests/:id", async (req, res) => {
      ...
      try {
        ...
        if (result.modifiedCount === 0) {
          return res
            .status(400)
            .send({ message: "Status already set to served" });
        }

        res
          .status(200)
          .send({ message: "Meal request status updated to served" });
      } catch (error) {
        console.error("Error updating meal request:", error);
        res
          .status(500)
          .send({ message: "Failed to update meal request status" });
      }
    });
    ...
    app.post("/meals/:id/review", async (req, res) => {
      ...
      if (meal.reviews.some((review) => review.email === email)) {
        return res
          .status(400)
          .send({ message: "User already reviewed the meal" });
      }
    ...
    app.patch("/meals/:id", async (req, res) => {
      ...
      res
        .status(200)
        .send({ message: "Meal updated successfully", updatedMeal });
    });
    ...
    app.delete("/meals/:mealId", async (req, res) => {
      ...
      try {
        ...
        if (admin) {
          admin.mealsAdded = admin.mealsAdded.filter(
          ...
          await adminCollection.updateOne(
            ...
            {
              $set: {
                mealsAdded: admin.mealsAdded,
                noOfMealsAdded: admin.noOfMealsAdded,
              },
            }
          );
        }
    ...
    app.post("/meals/from-upcoming", async (req, res) => {
      ...
      if (!upcomingMealId) {
        return res
          .status(400)
          .send({ message: "Upcoming meal ID is required" });
      }

      // Retrieve the upcoming meal from the database
      const upcomingMeal = await upcomingMealCollection.findOne({
        _id: new ObjectId(upcomingMealId),
      });
    ...
    app.post("/upcoming-meals", async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .send({ message: "Failed to add upcoming meal. Try again." });
      }
    });
